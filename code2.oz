...
%Nécessite: /
   ...
   fun{IsExtendedNote Note}
	 ...
	 if ((N == a orelse N == b orelse N == c orelse N == d orelse
	      N == e orelse N == f orelse N  == g)
	     andthen {IsInt O} andthen {IsBool S} andthen {IsFloat D}
	     andthen {IsAtom I})
	 then if  (N == e andthen S) orelse (N == b andthen S)
	      then false
	      else true
	      end
	 else false
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Renvoie le nombre de secondes que dure Partition.
% DurMaxChord renvoye la note qui a la plus grande durée dans l'accord
% Entrée de DurMaxChord :  Chord - format d'un ExtendedChord
%                          Acc - Float

% Entrée: Partition - format d'une Partition
% Necessite : IsNote NoteToExtended IsExtendedNote ChordToExtended IsChord IsExtendedChord


   fun{Longueur Partition}
      fun{DurMaxChord Chord Acc}
	 case Chord
	 of nil then Acc
	 [] H|T then
	    if H.duration>Acc then {DurMaxChord T H.duration}
	    else {DurMaxChord T Acc}
	    end
	 end
      end
	 ...
	 of nil then Acc
	 [] H|T then
	    if {IsNote H} then {Longueur T Acc+{NoteToExtended H}.duration}
	    elseif {IsExtendedNote H} then {Longueur T Acc+H.duration}
	    elseif {IsChord H} then
	       {Longueur T Acc+{DurMaxChord {ChordToExtended H} 0.0}}
	    elseif {IsExtendedChord H} then
	       {Longueur T Acc+{DurMaxChord H 0.0}}
	    end
	 end
      end
   in
      {Longueur Partition 0.0}
   end
   
   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Renvoie une partition où la durée de chaque note est multipliée par un
% facteur NbrSec/Durée totale de la partition pour que la durée totale de
% la partition  soit remplacée par le NbrSec


%Entrée: NbrSec - Float
%	 Partition - format de Partition	
% Necessite : Longueur Merge


   fun{Duration NbrSec Partition}
      local
	 DurPart = {Longueur Partition}
	 NewDuration = NbrSec/DurPart
      in
	 {Stretch NewDuration Partition}
      end
   end
